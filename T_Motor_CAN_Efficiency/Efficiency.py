...
# v_min always 0
speed_test_array = np.linspace(12,1,10)
current_test_array = np.linspace(14,1,10)
...
with open("Measuring_efficiency_{}_A_antagonist{}.csv".format(iq_antagonist,time.time()),'w') as fd:
    ...
    writer.writerow(["timestamp (epoch)", "loop time (s)", "des current", "velocityM (Rad/S)", "Futek Torque (Nm)","v_busM", "i_busM", "v_qM", "i_qM", "v_dM", "i_dM", "accelM", "velocityL (Rad/S)", "v_busL", "i_busL", "v_qL", "i_qL", "v_dL", "i_dL", "accelL"])
    ...
    with TMotorManager_servo_serial(motor_type='AK10-9', port = '/dev/ttyUSB1', baud=961200) as dev:  # Driven, AK10-9
        with TMotorManager_servo_serial(motor_type='AK80-9', port = '/dev/ttyUSB0', baud=961200) as ser: # Driving AK80-9
            with ADC_Manager('ADC_backup_log.csv') as adc:                                  # FUTEK
                ...
                #for speed in speed_test_array:

                    adc.update()
                    i = 0
                    speed = speed_test_array[i]
                    t_next = step_duration + int(time.time())
                    print("testing with: {} rad\s".format(speed))
                    time.sleep(0.1)
                    #for t in loop:
                    while True:
                        speed = speed_test_array[i]
                        ...
                        if i == len(current_test_array)/2 and flag:
                            start = time.time()
                        ...
                        ser.set_output_velocity_radians_per_second(speed)
                        ...
                        writer.writerow([time.time(), 
                                            t, 
                                            speed, 
                                            ser.get_output_velocity_radians_per_second(), 
                                            volt_to_torque(adc.volts, bias=bias), 
                                            ser.get_voltage_bus_volts(), 
                                            ser.get_current_bus_amps(), 
                                            ser.get_voltage_qaxis_volts(), 
                                            ser.get_current_qaxis_amps(), 
                                            ser.get_voltage_daxis_volts(), 
                                            ser.get_current_daxis_amps(), 
                                            ser.get_output_acceleration_radians_per_second_squared(),
                                            dev.get_output_velocity_radians_per_second(), 
                                            dev.get_voltage_bus_volts(), 
                                            dev.get_current_bus_amps(), 
                                            dev.get_voltage_qaxis_volts(), 
                                            dev.get_current_qaxis_amps(), 
                                            dev.get_voltage_daxis_volts(), 
                                            dev.get_current_daxis_amps(), 
                                            dev.get_output_acceleration_radians_per_second_squared()])
                                            
                        #print(t)
                        
                        print(ser.get_mosfet_temperature_celsius()," , ",dev.get_mosfet_temperature_celsius()," , ",speed," , ",current_test_array[i]," , ",ser.get_current_daxis_amps())
                        #writer.writenow(ser.parse_motor_parameters_async())   
